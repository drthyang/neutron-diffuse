...
This is the backend-less twin of :mod:`ndiff.server` for the GitHub Pages build.
...
:mod:`ndiff.server` (``volumes``, ``deltapdf``, ``consistency``, ``datasets``,
``params``).  Those import only numpy/scipy/h5py/ndiff — all available in Pyodide
— which is why :mod:`ndiff.server` imports ``create_app`` lazily.
...
import ndiff
from ndiff.pipeline import STAGES, run_pipeline
from ndiff.server import consistency as _cons
from ndiff.server import datasets as _ds
from ndiff.server import deltapdf as _dpdf
from ndiff.server import volumes as _vol
from ndiff.server.config import ServerConfig
from ndiff.server.params import build_params

if TYPE_CHECKING:
    from ndiff.server.schemas import PipelineRunRequest
...
def load_input(name: str, tmp_path: str) -> str:
    ...
    Copies it to ``raw/<stem>.nxs`` (``ndiff.load`` content-detects Mantid vs
    ndiff-HDF5, so the ``.nxs`` extension is fine for either) and returns the
...
def make_demo_input(n: int = 24) -> str:
    ...
    vol = ndiff.core.HKLVolume.from_arrays(
...
def run(
    ...
    *stages_csv* is a comma-separated subset of :data:`ndiff.pipeline.STAGES`
