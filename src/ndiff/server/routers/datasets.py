...
def _choose_directory(initial: Path) -> Path | None:
    ...
    if system == "Darwin":
        ...
        script = (
            ...
            "try\n"
            '  set chosenFolder to choose folder with prompt "Choose neutron-diffuse data folder" '
    ...
    if system == "Linux" and shutil.which("zenity"):
        proc = subprocess.run(
            [
                ...
                "--directory",
                f"--filename={initial}/",
                "--title=Choose neutron-diffuse data folder",
            ],
            check=False,
            capture_output=True,
            text=True,
...
def browse_data_root(
    ...
    if request.headers.get("x-ndiff-local") != "1":
