...
router (:mod:`ndiff.server.routers.pipeline`) and the in-browser bridge
(:mod:`ndiff.webbridge`, under Pyodide) can share one request → ``PipelineParams``
...
from ndiff.pipeline import PipelineParams
