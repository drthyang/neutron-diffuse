...
Slices are extracted with the *same* :func:`ndiff.visualization.extract_slice`
primitive the matplotlib viewers use, so the web viewer is pixel-for-pixel
consistent with them.  A loaded :class:`~ndiff.core.HKLVolume` is ~130 MB, so a
...
def load_volume(path: Path) -> HKLVolume:
    """Load an ndiff/Mantid volume, caching by ``(path, mtime)``."""
    ...
    with _lock:
        ...
        if vol is not None:
            ...
            return vol
    vol = ndiff.load(path)  # heavy I/O outside the lock
    with _lock:
        _cache[key] = vol
        _cache.move_to_end(key)
