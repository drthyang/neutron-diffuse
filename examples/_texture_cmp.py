"""Compare azimuthal texture models for 0kl ring removal on a chosen H slice."""
import matplotlib
matplotlib.use("Agg")
...
variants = [
    ("q.02 fourier3", dict(q_step=0.02, texture_model="fourier", n_fourier=3,
                           texture_ridge=0.3)),
    ("q.0025 fourier3", dict(q_step=0.0025, texture_model="fourier", n_fourier=3,
                            texture_ridge=0.3)),
    ("q.0025 patch", dict(q_step=0.0025, texture_model="patch", n_fourier=3,
                         texture_ridge=0.3)),
    ("q.0025 f3 ridge0", dict(q_step=0.0025, texture_model="fourier", n_fourier=3,
                             texture_ridge=0.0)),
    ("q.0025 f6 ridge0", dict(q_step=0.0025, texture_model="fourier", n_fourier=6,
                             texture_ridge=0.0)),
    ("q.0025 f6 ridge.1", dict(q_step=0.0025, texture_model="fourier", n_fourier=6,
                              texture_ridge=0.1)),
]
...
print(" ring    data  " + "  ".join([f"{label:>15}" for label, _ in fits]))
dprof = medprof(d.data)
signed_totals = np.zeros(len(fits))
abs_totals = np.zeros(len(fits))
neg_totals = np.zeros(len(fits))
for q0 in ring_centers:
    b = int(np.argmin(np.abs(qc - q0)))
    signed = []
    for i, (_, I_ring) in enumerate(fits):
        rp = medprof(d.data - I_ring)
        base = np.nanpercentile(rp[max(0, b - 12):b + 12], 20)
        resid = rp[b] - base
        signed.append(resid)
        signed_totals[i] += resid
        abs_totals[i] += abs(resid)
        neg_totals[i] += max(0.0, -resid)
    print(f"  {q0:5.2f}  {dprof[b]:6.3f}  "
          + "  ".join([f"{x:15.4f}" for x in signed]))
print(" signed leftover sum      "
      + "  ".join([f"{x:15.4f}" for x in signed_totals]))
print(" absolute leftover sum    "
      + "  ".join([f"{x:15.4f}" for x in abs_totals]))
print(" negative trough sum      "
      + "  ".join([f"{x:15.4f}" for x in neg_totals]))
print(" off-ring median removed  "
      + "  ".join([f"{np.median(I[off_ring]):15.5f}" for _, I in fits]))
print(" off-ring p95 removed     "
      + "  ".join([f"{np.percentile(I[off_ring], 95):15.5f}" for _, I in fits]))

fig, axes = plt.subplots(2, 3, figsize=(15, 10))
for ax, (label, I_ring) in zip(axes.ravel(), fits):
...
out = f"examples/_texture_cmp_{tag}.png"
