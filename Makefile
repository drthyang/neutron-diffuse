# neutron-diffuse — common dev tasks.
#
# The web UI is a single React app (web/) built into two gitignored targets:
#   make ui        -> src/ndiff/server/static  (served by native `ndiff-web`)
...
# or the running UI will be stale (native `ndiff-web` warns at startup if so).
...
ui: ## Rebuild the native (API-mode) SPA served by `ndiff-web`
